...
async function updateFacebookOAuthRedirectUri() {
    try {
        ...
        const tokenResponse = await fetch(`https://graph.facebook.com/oauth/access_token?client_id=${facebookConfig.appId}&client_secret=${facebookConfig.appSecret}&grant_type=client_credentials`);
        const { access_token } = await tokenResponse.json();
        // Update the OAuth redirect URIs
        const response = await fetch(`https://graph.facebook.com/v19.0/${facebookConfig.appId}`, {
            method: 'POST',
            headers: {
